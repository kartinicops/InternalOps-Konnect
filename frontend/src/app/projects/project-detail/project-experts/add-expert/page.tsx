...
export default function AddExpert() {
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      // If a project ID was provided, add the expert to the project pipeline
      if (projectId) {
        await API.post("/project_pipeline/", {
          expert_id: expertId,
          project_id: parseInt(projectId),
          user_id: 6, // This could be dynamically fetched in a real application
        })
        
        toast.success("Expert added to project pipeline!")
        setTimeout(() => router.push(`/projects/project-detail?id=${projectId}&tab=experts`), 2000)
      } else {
    ...
    <div className="min-h-screen bg-gray-100">
      ...
      <div className="container mx-auto py-8 px-4 max-w-5xl">
        <div className="flex items-center mb-8">
          <Button variant="ghost" onClick={handleBack} className="mr-4 text-blue-600 hover:text-blue-800 hover:bg-blue-50">
            <ArrowLeft className="h-5 w-5 mr-1" />Back
          </Button>
          <h1 className="text-3xl font-bold text-blue-600">
            {projectId ? "Add Expert to Project" : "Add New Expert"}
          </h1>
        </div>
